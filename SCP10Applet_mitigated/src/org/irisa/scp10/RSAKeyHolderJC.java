...
public class RSAKeyHolderJC {
    ...
    private Cipher rsa_raw;
    private Cipher rsa_oaep;
    ...
    public RSAKeyHolderJC(short modulusSize) {
        ...
        rsa_raw = Cipher.getInstance(Cipher.ALG_RSA_NOPAD, false);
        rsa_oaep = Cipher.getInstance(Cipher.ALG_RSA_PKCS1_OAEP, false);
        isPrivate = false;
    }
    ...
    public short publicKeyOperation(byte[] buffer, short bOffset, short bLen, byte encryptionMode) {
        rsa_raw.init(pub, encryptionMode);

        return rsa_raw.doFinal(buffer, bOffset, bLen, buffer, bOffset);
    }
    ...
    public short privateKeyOperation(byte[] buffer, short bOffset, short bLen) {
        ...
        rsa_raw.init(priv, Cipher.MODE_DECRYPT);

        return rsa_raw.doFinal(buffer, bOffset, bLen, buffer, bOffset);
    }

    /**
     * Decrypt a ciphertext, assuming OAEP padding
     * @param buffer
     ...
     */
    public short DecryptOAEP(byte[] buffer, short bOffset, short bLen) {
        rsa_oaep.init(priv, Cipher.MODE_DECRYPT);
        short mLen = rsa_oaep.doFinal(buffer, bOffset, bLen, buffer, (short) (bOffset+bLen));

        // Copy the payload at the beginning of the buffer
        Util.arrayCopy(buffer, (short) (bOffset+bLen), buffer, bOffset, mLen);
     ...
     * the offset of the data to hash and the data to embed are distinguished.
     * After producing the signature s, the value min(s, n-s) is stored at offset
     * bOffset in buffer.
     *
     * Since the SignatureMessageRecovery class does not comply with SCP10 specification,
