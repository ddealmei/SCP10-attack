...
class RsaKeyHolder(object):
    ...
    def encrypt_deterministic_pkcs1(self, data):
        if len(data) > self.size_bytes - 11:
            raise ValueError("Message to big to be encrypted.")
        payload = [00, 2]
        payload += [0xFF for _ in range(self.size_bytes-len(data)-3)]
        payload += [0]
        payload += data

        return self.public_key_op(payload)
